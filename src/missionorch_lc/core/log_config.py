...
每次调用 `setup_logging()` 会在 `logs/` 目录下生成带时间戳的独立日志文件：
  - `debug_<timestamp>.log`          全量日志（含 Agent 输入输出）
  - `interactions_<timestamp>.log`   仅 Agent 输入输出
"""
...
def setup_logging(log_dir: str = "logs", console_level: int = logging.INFO) -> str:
    ...
    interaction_logger.propagate = True

    interaction_file_handler = logging.FileHandler(
        str(interaction_log_file), mode="w", encoding="utf-8"
    )
